  ...
  struct rtpkt *rcvdpkt;
{
    ...
    int min_costs_from_dst[4];
    for (int i = 0; i < 4; i++)
        min_costs_from_dst[i] = rcvdpkt->mincost[i];
    ...
    for (int i = 0; i < 4; i++)
    {
        int new_cost = dt0.costs[src_node][src_node] + min_costs_from_dst[i];
        if (new_cost < INF)
